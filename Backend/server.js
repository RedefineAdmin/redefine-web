...
function initializeSchema() {
    db.serialize(() => {
        ...
        db.run(`CREATE TABLE IF NOT EXISTS replies (
            ...
            name TEXT, username TEXT, avatar TEXT, color TEXT, verified INTEGER DEFAULT 0,
            time TEXT, text TEXT, views INTEGER DEFAULT 0, likes INTEGER DEFAULT 0, 
            hasLiked INTEGER DEFAULT 0, rt INTEGER DEFAULT 0, hasRT INTEGER DEFAULT 0,
        ...
        db.run(`ALTER TABLE replies ADD COLUMN hasLiked INTEGER DEFAULT 0`, () => {});
        db.run(`ALTER TABLE replies ADD COLUMN rt INTEGER DEFAULT 0`, () => {});
        db.run(`ALTER TABLE replies ADD COLUMN hasRT INTEGER DEFAULT 0`, () => {});
    });
}
...
app.post('/api/user/avatar', (req, res) => {
    ...
    db.run(`UPDATE users SET avatar = ? WHERE username = ?`, [avatar, username], function(err) {
        if (err) return res.status(500).json({ success: false, message: "Failed to upload avatar" });
...
app.get('/api/tweets/:id/replies', (req, res) => {
    ...
    db.all(`SELECT * FROM replies WHERE tweetId = ?`, [tweetId], (err, rows) => {
        if (err) return res.json({ success: true, replies: [] });
...
app.post('/api/tweets/:id/replies', (req, res) => {
    ...
    db.run(`CREATE TABLE IF NOT EXISTS replies (id INTEGER PRIMARY KEY AUTOINCREMENT, tweetId INTEGER, name TEXT, username TEXT, avatar TEXT, color TEXT, verified BOOLEAN, time TEXT, text TEXT, views INTEGER DEFAULT 0, likes INTEGER DEFAULT 0, hasLiked INTEGER DEFAULT 0, rt INTEGER DEFAULT 0, hasRT INTEGER DEFAULT 0, timestamp DATETIME DEFAULT CURRENT_TIMESTAMP)`, () => {
